...
export default function App() {
  ...
  const addNoteComment = (comment: RuntimeNoteComment, sourceText: string) => {
    ...
    setNodes((previous) => previous.map((node) => {
      ...
      if (comment.kind === 'highlight') return updated;
      return {
        ...updated,
  ...
  const deleteNoteComment = (commentId: string) => {
    ...
    setNodes((previous) => previous.map((node) => {
      ...
      const weakpoints = node.weakpoints.filter((weakpoint) => weakpoint.source_conversation_id !== sourceId);
      return { ...node, weakpoints, status: node.status === 'weak' && weakpoints.length === 0 ? 'learned' : node.status };
    }));
